...
function ContractSelect({ save, startContract, learningLevel, setLearningLevel }: { save: SaveData; startContract: (index: number) => void; learningLevel: LearningLevel; setLearningLevel: (level: LearningLevel) => void }) {
  ...
  return <section className={styles.select}>
    <div className={styles.selectIntro}><p>DIRECTOR DEMO / 90-SECOND VERTICAL SLICE</p><h1>MAKE THE PART.<br/><em>PROTECT THE EDGE.</em></h1><span>Remove the silver stock. Keep the glowing part untouched. One flagship contract proves the complete learn–cut–inspect–improve loop.</span><div className={styles.heroActions}><button onClick={() => startContract(0)}><Play/> START FLAGSHIP CONTRACT</button><small>NO ACCOUNT · FICTIONAL TRAINING VALUES · KEYBOARD, MOUSE &amp; TOUCH</small></div></div>
